...
### Installation
Create an instance of Ubuntu 14.04 server in your favourite VM.  From /root, download and run the ntp_builder.pl script.  It may be useful to run *script* to capture the output.  Apache and necessary perl modules will be installed (including debug tools).  crontab will be modified to run the ntp_checker every hour.  The crontab script generated remains as /root/crontabadditon; it can be deleted.  
...
`dpkg-reconfigure -f noninteractive tzdata` is executed as part of the build to set/check the time zone.  This should be UTC.  
If using a dedicated VM specific for ntp_checker, leave in /root, otherwise change the paths/crontab as necessary.

Variables in ntp_chekcer for the email settings need to be modified as required:
