...
### Installation
Create an instance of Ubuntu 14.04 server in your favourite VM.  From /root, download and run the ntp_builder.pl script.  It may be useful to run *script* to capture the output.  Apache and necessary perl modules will be installed (including a few perl debug tools).
`wget https://raw.githubusercontent.com/fss1/ntp_checker/master/perl/ntp_builder.pl`
`perl ntp_builder.pl`
Download and run the ntp_builder.pl script Download the ntp_checker.pl script and make it executable. The build script configures cron to run this from /root  
