...
# For single stat, +ve version of time_offset; row name defined here: 
my $poffset='poffset';
# Suggested value for poffset:
# 0 > 0.1 Green, 0.1 > 0.3 Orange, 0.3 > Red; failure to respond
# should have some high value such as 666 to denote this
...
system ("curl -s -i -X POST \"http://localhost:8086/write?db=$insert_array[0]\" --data-binary \"$insert_array[1],server=$insert_array[2] value=$insert_array[3]\" > /dev/null");
