...
# variable to hold max positive offset found
my $maxpos = 0;

# variable to hold max minimum offset found
my $maxneg = 0;
...
system(
"curl -s -i -X POST \"http://localhost:8086/write?db=$db_name\" --data-binary \"maxpos value=$maxpos\" > /dev/null"
);
system(
"curl -s -i -X POST \"http://localhost:8086/write?db=$db_name\" --data-binary \"maxneg value=$maxneg\" > /dev/null"
);
